...
// Define User Schema
const userSchema new mongoose. Schema ({ name: { type: String, required: true }, email: { type: String, required: true, unique: true }, password: { type: String, required: true }, upi_id: { type: String, unique: true }, balance: { type: Number}
});
// Create User Model
...
// Signup Route app.post('/api/signup', async (req, res) => {
try {
...
if (luser || user.password !== password) {
...
T. const transaction = new Transaction(sender_upi_id, receiver_upi_id, amount });
...
$or: [{ sender_upi_id: upi_id}, { receiver_upi_id: upi_id}] }).sort({ timestamp: -1));
